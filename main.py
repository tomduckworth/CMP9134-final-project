...
class User:
    ...
    def login(self) -> bool:
        ...
        logging.info(f"User '{self.username}' (Role: {self.role}) securely logged in.")
        return True
...
class AuditLogger:
    ...
    def record_mission(self, commander: str, x: int, y: int, status: MissionStatus) -> None:
        """Inserts an immutable mission record."""
        timestamp = datetime.datetime.now().isoformat(sep=" ", timespec="seconds")
        
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT INTO logs (timestamp, commander, target_x, target_y, status) VALUES (?, ?, ?, ?, ?)",
...
class RobotClient:
    ...
    def get_telemetry(self) -> dict:
        ...
        try:
            ...
            self.logger.warning("Robot unreachable for telemetry. Returning simulated offline data.")
            return {"x": "unknown", "y": "unknown", "status": "offline"}

    def transmit_move(self, x: int, y: int) -> None:
        """Sends payload to API. Raises specific requests exceptions on failure."""
        payload = {"x": x, "y": y}
...
class GroundControlStation:
    ...
    def _validate_safety(self, x: int, y: int) -> None:
        if not (0 <= x <= self.config.grid_size and 0 <= y <= self.config.grid_size):
            raise SafetyBoundaryError(f"Target ({x}, {y}) exceeds authorized safety grid (0-{self.config.grid_size}).")

    def _verify_authorization(self, user: User, required_role: str) -> None:
        if not user.is_authenticated:
            raise AuthenticationError(f"Access Denied: User '{user.username}' is not logged in.")
        if user.role != required_role:
            raise AuthenticationError(f"Access Denied: '{user.username}' lacks '{required_role}' privileges.")
    ...
    def execute_move_command(self, user: User, x: int, y: int) -> bool:
        self.logger.info(f"Command received from {user.username}: MOVE TO ({x}, {y})")
        
        try:
            self._verify_authorization(user, required_role="Commander")
        except AuthenticationError as e:
            self.logger.warning(f"SECURITY INTERVENTION: {e}")
            self.db.record_mission(user.username, x, y, MissionStatus.UNAUTHORIZED)
            return False
        ...
        try:
            ...
            self.db.record_mission(user.username, x, y, MissionStatus.REJECTED_BOUNDS)
            return False
        ...
        try:
            ...
            self.db.record_mission(user.username, x, y, MissionStatus.API_ERROR)
            return False
        except requests.exceptions.RequestException as e:
            self.logger.error(f"Connection to Robot severed: {e}")
            self.db.record_mission(user.username, x, y, MissionStatus.CONNECTION_ERROR)
            return False
